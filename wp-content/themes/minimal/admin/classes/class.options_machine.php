...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		global $smof_output, $smof_details, $sd_data;
		if (empty($options))
			return;
		if (empty($sd_data))
			$sd_data = of_get_options();
		$data = $sd_data;
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($sd_data) || !empty($data)){
			
				if (array_key_exists('id', $value) && !isset($sd_data[$value['id']])) {
					$sd_data[$value['id']] = $value['std'];
				...
				if (array_key_exists('id', $value) && !isset($smof_details[$value['id']])) {
					$smof_details[$value['id']] = $sd_data[$value['id']];
				}
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
				...
				if (array_key_exists("fold",$value)) {
					if (isset($sd_data[$value['fold']]) && $sd_data[$value['fold']]) {
						$fold="f_".$value['fold']." ";
					} else {
			...
			//if (!isset($sd_data[$value['id']]) && $value['type'] != "heading")
			//	continue;
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes($sd_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						...
						if (!is_numeric($select_ID))
							$theValue = $select_ID;
						$output .= '<option id="' . $select_ID . '" value="'.$theValue.'" ' . selected($sd_data[$value['id']], $theValue, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($sd_data[$value['id']]);			
						$output .= '<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';		
				break;
				
				//radiobox option
				case "radio":
					$checked = (isset($sd_data[$value['id']])) ? checked($sd_data[$value['id']], $option, false) : '';
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($sd_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($sd_data[$value['id']])) {
						$sd_data[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($sd_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					(isset($sd_data[$value['id']]))? $multi_stored = $sd_data[$value['id']] : $multi_stored="";
				...
				case "color":
					...
					$output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $sd_data[$value['id']] . '"' . $default_color .' />';
				...
				case 'typography':
					...
					$typography_stored = isset($sd_data[$value['id']]) ? $sd_data[$value['id']] : $value['std'];
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $sd_data[$value['id']];
				...
				case 'images':
					...
					$select_value = (isset($sd_data[$value['id']])) ? $sd_data[$value['id']] : '';
				...
				//check for custom css file permissions
				case "permissions":
					$file_path = get_stylesheet_directory() . '/framework/css/custom-styles.css';;
					if (is_writeable($file_path)) {
						$persmission = '';
						} else {
						$persmission = '<div class="permissions"> <strong><span>NOTE:</span> The file "custom-styles.css" located in the theme\'s folder > framework > css does not have write permission. Make sure the file is writable before your custom styling can be saved. <br/><br/> If you don\'t know how to set file permissions <a target="_blank" href="http://skat.tf/how-to-set-themes-file-permissions-via-ftp">click here!</a></strong></div>';
		}
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($sd_data[$value['id']]) && !empty($sd_data[$value['id']]) ? $sd_data[$value['id']] : '';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'sd-framework').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($sd_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
				...
				case 'select_google_font':
					...
					foreach ($value['options'] as $select_key => $option) {
						$output .= '<option value="'.$select_key.'" ' . selected((isset($sd_data[$value['id']]))? $sd_data[$value['id']] : "", $option, false) . ' />'.$option.'</option>';
					} 
					$output .= '</select></div>';
					...
					$hide = " hide";
					if ($sd_data[$value['id']] != "none" && $sd_data[$value['id']] != "")
						$hide = "";
				...
				case 'sliderui':
					...
					$s_val  = stripslashes($sd_data[$value['id']]);
				...
				case 'switch':
					...
					$output .= '<p class="switch-options">';
						...
						$output .= '<input type="hidden" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
						$output .= '<input type="checkbox" id="'.$value['id'].'" class="'.$fold.'checkbox of-input main_checkbox" name="'.$value['id'].'"  value="1" '. checked($sd_data[$value['id']], 1, false) .' />';
				...
				case "media":
					...
					$u_val = '';
					if($sd_data[$value['id']]){
						$u_val = stripslashes($sd_data[$value['id']]);
					}
