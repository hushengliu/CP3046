...
function optionsframework_admin_init() 
{
	...
	global $of_options, $options_machine, $sd_data, $smof_details;
	...
	if (empty($sd_data['smof_init'])) { // Let's set the values if the theme's already been active
...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$sd_data = of_get_options(BACKUPS);

		of_save_options($sd_data);
		
		generate_options_css($sd_data); //generate static css file
	...
	elseif($save_type == 'import_options'){
		...
		$sd_data = unserialize(base64_decode($_POST['data'])); //100% safe - ignore theme check nag
		of_save_options($sd_data);
	...
	elseif ($save_type == 'save')
	{
		...
		wp_parse_str(stripslashes($_POST['data']), $sd_data);
		unset($sd_data['security']);
		unset($sd_data['of_save']);
		of_save_options($sd_data);
